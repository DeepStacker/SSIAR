...
def align_page_hierarchical(img: np.ndarray, template: np.ndarray) -> Tuple[np.ndarray, Dict[str, np.ndarray], str]:
    ...
    if globally_aligned is None:
        # Try 180-degree rotation (in case page was scanned upside down)
        img_180 = cv2.rotate(img, cv2.ROTATE_180)
        globally_aligned, global_method = align_images_fallback(img_180, template)
        if globally_aligned is not None:
            print(f"Alignment: Detected 180-degree rotated scan. Aligned successfully using {global_method}!")
            img = img_180
