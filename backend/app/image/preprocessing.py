...
def select_and_apply_preprocessing(img: np.ndarray, q: Dict[str, Any]) -> np.ndarray:
    ...
    High-quality scans (>= 80) take a fast path: CLAHE + sharpening only.
    """
    out = img.copy()

    # Fast path: high-quality scans need minimal preprocessing
    if q["quality"] >= 80 and q["contrast"] >= 45 and q["shadow"] <= 15.0 and q["noise"] <= 8.0:
