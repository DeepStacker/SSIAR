...
def detect_checkboxes(
    ...
    for idx, (y0, y1) in enumerate(y_ranges):
        ...
        # 3. Find the column with the strongest ink signal
        sorted_idx = np.argsort(col_ink_counts)[::-1]
        best_idx = int(sorted_idx[0])
        best_ink = int(col_ink_counts[best_idx])
        second_best_ink = int(col_ink_counts[int(sorted_idx[1])]) if len(sorted_idx) > 1 else 0
        
        # Require minimum absolute ink to consider any column selected
        if best_ink < 12:
            results[q_num] = (0, CheckboxState.EMPTY, 1.0, col_states, False)
            continue
        
        # Determine state based on relative ink signal:
        #   ratio > 2.5 → clean tick (strong signal)
        #   ratio 1.8–2.5 → partial (moderate, flag for review)
        #   ratio < 1.8 or second column also strong → double mark
        ratio = best_ink / max(second_best_ink, 1)
        second_strong = bool(second_best_ink > 12 and second_best_ink > best_ink * 0.6)
        
        if second_strong:
            final_state = CheckboxState.DOUBLE
            confidence = 0.50
        elif ratio > 2.5:
            final_state = CheckboxState.TICK
            confidence = float(min(0.98, 0.5 + ratio * 0.15))
        elif ratio > 1.8:
            final_state = CheckboxState.PARTIAL
            confidence = float(min(0.85, 0.4 + ratio * 0.15))
        else:
            final_state = CheckboxState.PARTIAL
            confidence = 0.50
