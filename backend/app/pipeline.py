...
from app.modules.checkbox import detect_checkboxes as checkbox_detector, CheckboxState
...
def align_page_orb(img, page_num, templates_dir):
    ...
    from app.modules.alignment import align_images_fallback
...
def detect_consent(aligned_img):
    ...
    from app.modules.checkbox import _analyze_checkbox
...
def _deskew_image(img):
    from app.modules.preprocessing import detect_skew_angle, deskew_image
...
def repair_illumination(img):
    from app.modules.preprocessing import remove_shadows
    return remove_shadows(img)

def repair_noise(img, classification_type):
    from app.modules.preprocessing import apply_denoising
